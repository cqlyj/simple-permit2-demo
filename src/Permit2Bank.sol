...
contract Permit2Bank is EIP712 {
    ...
    bytes32 constant WITNESS_BATCH_FULL_TYPEHASH =
        keccak256(
            "PermitBatchWitnessTransferFrom(TokenPermissions[] permitted,address spender,uint256 nonce,uint256 deadline,Witness witness)TokenPermissions(address token,uint256 amount)Witness(address user)"
        );
    ...
    function getPermitBatchWitnessTransferFromHash(
        ISignatureTransfer.PermitBatchTransferFrom calldata permitBatchFrom,
        address spender,
        address user
    ) external view returns (bytes32) {
